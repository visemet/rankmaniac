...
def handle_grade(args):
    ...
    i = 0
    while len(team_ids) > 0: # length is NOT constant
        i %= len(team_ids)
        team_id = team_ids[i]
        try:
            unbuff_stdout.write('.')
            ...
            if job.is_done(jobdesc=jobdesc):
                runtime = job.compute_job_time(jobdesc=jobdesc)
                penalty = job.compute_penalty(jobdesc=jobdesc)
            ...
            elif not job.is_alive(jobdesc=jobdesc):
        ...
        # Interrupt waiting on jobs to return to the prompt
        except KeyboardInterrupt:
            print('')
            break

        # Swallow any exceptions generated by team and skip over
